...
Header::Header(int x, bool* add_on_p) // tworzie paska i ustawianie tekstur
{
    add_on = add_on_p;
    size = x;
...
void Header::draw(sf::RenderWindow* window) // rysowanie 
{
    window->draw(bar);
    window->draw(save);
    window->draw(back);
...
void Header::press(int x) // dzi³ania przy naciœniêciu na pasek 
{
